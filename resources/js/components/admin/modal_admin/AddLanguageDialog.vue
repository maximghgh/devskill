...
async function submit() {
  ...
  try {
    const { data } = await axios.post("/api/languages", { name: name.value });

    globalNotification.categoryMessage = data?.message || "Категория создана";
    ...
    console.error(e);
    error.value = "Ошибка. Проверьте название.";
