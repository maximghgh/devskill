...
const props = defineProps({
  ...
  topic: { type: Object, default: null }, // сюда передаем тему на редактирование
});
...
watch(
  () => props.topic,
  () => {
    if (props.modelValue) fillFromTopic();
  }
);
...
async function submit() {
  error.value = "";
  if (!form.value.id) {
    error.value = "Не выбрана тема для редактирования";
    return;
  }

  loading.value = true;
  try {
    // ⚠️ если у тебя не /api/admin, а /admin — замени URL ниже
    const { data } = await axios.patch(`/api/admin/topics/${form.value.id}`, {
    ...
    error.value = "Ошибка обновления темы";
