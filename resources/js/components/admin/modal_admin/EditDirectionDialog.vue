        ...
        <div class="dialog__content">
          <div class="form__admin">
            ...
            <div class="dialog__btns">
              ...
              <button type="button" class="main__btn" @click="submit" :disabled="loading || !name.trim()">
...
const props = defineProps({
  ...
  direction: { type: Object, default: null }, // {id, name, description}
});
...
watch(
  ...
  (v) => {
    if (v) {
      name.value = props.direction?.name ?? "";
      description.value = props.direction?.description ?? "";
      error.value = "";
      loading.value = false;
    }
  }
...
async function submit() {
  if (!props.direction?.id) return;

  error.value = "";
  loading.value = true;

  try {
    const { data } = await axios.patch(`/api/directions/${props.direction.id}`, {
      name: name.value,
    ...
    emit("saved", data.direction ?? data);
    close();
  } catch (e) {
    console.error(e);
